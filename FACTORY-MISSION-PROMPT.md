...
> 🟢 **BUILD APPROACH — TRUE GREENFIELD (overrides all framing below).** Clone Check is a **from-scratch** codebase. Do **NOT** import, copy, fork, or transform the `github-repo-explorer` repo. Wherever this brief references that repo's files or line numbers (e.g. "`api.js:464`", "`HealthScore.js` is DOM-coupled at `:121`", "demote Pulse", "reuse the CSS tokens"), treat those strictly as **patterns to implement correctly from scratch** (bugs to avoid, behaviors to get right) — **not files to edit, reuse, or carry over.** The product is completely different from that explorer; build only Clone Check's own minimal surfaces. The §2 validation contract (behavioral assertions) is unchanged and remains the rubric.
